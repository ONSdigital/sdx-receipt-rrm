            ...
            if processed_ok:
                ...
                if delivery_count == settings.QUEUE_MAX_MESSAGE_DELIVERIES:
                    ...
                    if publish_to_retry_queue(body, delivery_count):
                        logger.info("Published to retry queue", tx_id=processor.tx_id, queue=settings.RABBIT_DELAY_QUEUE,
                                    delivery_count=delivery_count)
                        self.reject_message(basic_deliver.delivery_tag, tx_id=processor.tx_id)
                    else:
                        logger.error("Failed to publish to retry queue", tx_id=processor.tx_id,
                                     queue=settings.RABBIT_DELAY_QUEUE)
