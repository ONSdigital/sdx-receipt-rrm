...
class AsyncConsumer(object):
    ...
    def connect(self):
        ...
        while True:
            ...
            try:
                logger.info('Connecting to queue server', attempt=count)
                return pika.SelectConnection(pika.URLParameters(self._url),
                                             self.on_connection_open,
                                             stop_ioloop_on_close=False)
            except pika.exceptions.AMQPConnectionError as e:
                logger.error("Queue server connection error", exception=e)
                count += 1
                logger.error("Queue server connection sleep", no_of_seconds=count)
                time.sleep(count)
